...
def make_bricks(small, big, goal):
    if not isinstance(small, int) or not isinstance(big, int) or not isinstance(goal, int):
        print("Wrong type of one of the values. Expected string type.")
        quit()
    else:
        bricks = [True if small * 1 + big * 5 >= goal else False]
        return bool(bricks)
...
def fix_teen(n: int) -> int:
    if not isinstance(n, int):
        ...
        teen_numbers = [13, 14, 17, 18, 19]
        if n in teen_numbers:
            return 0
        return n
...
def round_sum(a, b, c):
    if not isinstance(a, int) or not isinstance(b, int) or not isinstance(c, int):
        ...
        return num_round(a) + num_round(b) + num_round(c)